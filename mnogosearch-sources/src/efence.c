...
allocateMoreSlots(void)
{
	...
	newAllocation = UdmMalloc(newSize);
...
extern C_LINKAGE void UdmFree(void * address) {
	Slot *	slot;
	Slot *	previousSlot = 0;
	Slot *	nextSlot = 0;
	...
	if ( allocationList == 0 )
		EF_Abort("UdmFree() called before first UdmMalloc().");
	...
	if ( !slot )
		EF_Abort("UdmFree(%a): address not from UdmMalloc().", address);
...
extern C_LINKAGE void * UdmRealloc(void * oldBuffer, size_t newSize) {
	void *	newBuffer = 0;
	...
	if ( oldBuffer ) {
		...
		if ( slot == 0 )
			EF_Abort(
			 "UdmRealloc(%a, %d): address not from UdmMalloc()."
			,oldBuffer
			,newSize);
...
extern C_LINKAGE void *UdmMalloc(size_t size) {
	if ( allocationList == 0 )
		initialize();	/* This sets EF_ALIGNMENT */
...
extern C_LINKAGE void *UdmCalloc(size_t nelem, size_t elsize) {
	size_t	size = nelem * elsize;
	void *	allocation = UdmMalloc(size);
 ...
 */
extern C_LINKAGE void *UdmValloc (size_t size) {
	return memalign(bytesPerPage, size);
}

/*
*/
extern C_LINKAGE char * UdmStrdup(const char *str) {
        size_t len;
        char *copy;

        len = strlen(str) + 1;
        if ((copy = UdmMalloc(len)) == NULL)
                return (NULL);
        memcpy(copy, str, len);
        return (copy);
}
